...
static bool read_request(const int fd, const uint8_t * request) {
    //0. lectura del primer hello
    uint8_t buff[256+1+1];
    buffer buffer;
    buffer_init(&buffer, N(buff), buff);
    struct request_parser request_parser;
    request_parser.request = request;
    request_parser.state = request_version;
    ...
    do {
        ...
        if(n > 0){
            buffer_write_adv(&buffer, n);
            const enum hello_state st = request_consume(&buffer, &request_parser, &error); //o es *error?
    ...
    /*if( ! hello_is_done(hello_parser.state, &error)){ no lo veo realmente necesario, para chequear
        error = true;
    }
    hello_parser_close(&hello_parser);*/
    return error;
...
socks5_handle_connection(const int fd, const struct sockaddr *caddr){
    ...
    hello_marshall(&b, r);
    if(sock_blocking_write(fd, &b) != 0) { //de netutils.h .c
        //error handling aca
    }
    ...
    enum socks_response_status status = status_general_SOCKLS_server_failure; // talvez no sea failure
